      ...
      <nav className="sticky top-0 z-50 bg-white border-b-4 border-black px-6 py-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center text-black">
          <motion.div 
            ...
            onClick={() => router.push('/')}
            className="text-3xl font-black tracking-tighter flex items-center gap-2 cursor-pointer text-black"
          >
              ...
              <>
                ...
                {adminItems.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`flex items-center gap-2 px-3 py-2 border-2 border-black bg-purple-100 hover:bg-purple-200 transition-all text-black ${pathname === item.href ? 'bg-purple-400 shadow-[2px_2px_0px_0px_#000]' : 'shadow-[2px_2px_0px_0px_#000]'}`}
                  >
                    {item.icon}
                    <span>{item.label}</span>
                  </Link>
                ...
                <>
                  ...
                  {adminItems.map((item) => (
                    <Link
                      ...
                      href={item.href}
                      onClick={() => setIsMenuOpen(false)}
                      className={`flex items-center gap-3 p-4 border-4 border-black bg-purple-100 text-black ${pathname === item.href ? 'bg-purple-400 shadow-none translate-x-1 translate-y-1' : 'shadow-[4px_4px_0px_0px_#000]'}`}
                    >
                      {item.icon}
                      {item.label}
                    </Link>
